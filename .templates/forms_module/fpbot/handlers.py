...
async def handle_cmds(fpbot: 'FunPayBot', event: NewMessageEvent):
    global new_forms
    this_chat = fpbot.account.get_chat_by_name(event.message.chat_name, True)
...
async def run_cycles(_):
    ...
    def run():
        ...
        stop_cycles_event.clear()
        
        # Ждём, пока инициализируется FunPay бот, прежде чем продолжить работу
        while not hasattr(get_funpay_bot(), "account"): 
            time.sleep(1)
