...
EXPORT_ALL_NAME = "Конфиг FunPay Universal.zip"
EXPORT_FULL_NAME = "Конфиг и данные FunPay Universal.zip"
SUPPORTED_EXTENSIONS = (".json", ".zip", ".rar")

SECRET_PATHS = [
    ("funpay", "api", "golden_key"),
    ("funpay", "api", "user_agent"),
    ("funpay", "api", "proxy"),
    ("telegram", "api", "token"),
    ("telegram", "api", "proxy"),
...
def import_file(path: str, keep_secrets: bool = True) -> tuple[list[str], list[tuple[str, str]], str | None]:
    ...
    :param keep_secrets: Оставить текущие секретные данные (Golden Key, токены, пароль).
