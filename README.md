...
- Базовый функционал:
  ...
  - Возможность добавления пользовательской авто-выдачи на лоты
...
<details>
  ...
  </br>Модуль - это папка, внутри которой находятся важные компоненты. Вы можете изучить строение модуля, опираясь на [шаблонный модуль](templates/test_module) **(не актуален, обращайтесь к @alleexxeeyy)**, но стоит понимать, что это лишь пример, сделанный нами.
  ...
  def handler_on_init():
      try:
          ...
          ModulesManager.disable_module(_module.uuid)
...
<details>
  ...
  Вы можете использовать класс `core.modules_manager.ModulesManager`, внутри которого описаны все небходимые методы для управления вашим модулем внутри кода (включение, выключение, перезагрузка...)
